...
*********************************************************/
pub fn poseidon_permutation<T: PrimeField>(state: Vec<T>, constants: &Constants<T>) -> Vec<T> {
    let mut new_state = state.to_vec();

    for i in 0..(constants.full_rounds + constants.partial_rounds) as usize {
        ark(&mut new_state, constants, i);
        sbox(&mut new_state, constants, i);
        linear_layer(&mut new_state, constants);
    }
